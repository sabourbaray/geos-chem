...
!
  ...
  TYPE, PUBLIC :: PhotContainer
     ...
     ! Renamed from Fast-JX/Cloud-J module variables for clarity
...
!
!cldj #ifdef CLOUDJ
!cldj     USE Cldj_Cmn_Mod,   ONLY : A_, AN_, W_, WX_, JVN_, N_, JXL_
!cldj #else
    USE CMN_FastJX_Mod, ONLY : A_, AN_, W_, WX_, JVN_, N_, JXL_
!cldj #endif
...
!
    ...
    ! Store certain values from Fast-JX/Cloud-J with more intuitive name
    Phot%nWLbins      = W_
    ...
    IF ( .not. Input_Opt%DryRun ) THEN
       ...
       ! Phot%RINDEX      (:)
       ALLOCATE( Phot%RINDEX(JVN_), STAT=RC )
       ...
       ! Phot%GC_Photo_Id (:)
       ALLOCATE( Phot%GC_Photo_Id(JVN_), STAT=RC )
       IF ( RC /= GC_SUCCESS ) THEN
       ...
       ! Phot%UVXFACTOR(:)
       ALLOCATE( Phot%UVXFACTOR(AN_), STAT=RC )
       IF ( RC /= GC_SUCCESS ) THEN
       ...
       ! Phot%QAA_AOD  (:)
       ALLOCATE( Phot%QAA_AOD(AN_), STAT=RC )
       IF ( RC /= GC_SUCCESS ) THEN
       ...
       ! Phot%WAA_AOD  (:)
       ALLOCATE( Phot%WAA_AOD(AN_), STAT=RC )
       IF ( RC /= GC_SUCCESS ) THEN
       ...
       ! Phot%PAA_AOD  (:)
       ALLOCATE( Phot%PAA_AOD(AN_), STAT=RC )
       IF ( RC /= GC_SUCCESS ) THEN
       ...
       ! Phot%RAA_AOD  (:)
       ALLOCATE( Phot%RAA_AOD(AN_), STAT=RC )
       IF ( RC /= GC_SUCCESS ) THEN
       ...
       ! Phot%SAA_AOD  (:)
       ALLOCATE( Phot%SAA_AOD(AN_), STAT=RC )
       IF ( RC /= GC_SUCCESS ) THEN
       ...
       ! Phot%ZPJ      (:,:,:,:)
       ALLOCATE( Phot%ZPJ( State_Grid%NZ, JVN_, State_Grid%NX, &
    ...
    IF ( .not. Input_Opt%DryRun ) THEN
       ...
       ALLOCATE( Phot%ODMDUST( State_Grid%NX, State_Grid%NY, &
                 State_Grid%NZ, Phot%NWVAA, NDUST), STAT=RC )
       IF ( RC /= GC_SUCCESS ) THEN
       ...
       ALLOCATE( Phot%RTODAER( State_Grid%NX, State_Grid%NY, &
                 State_Grid%NZ, Phot%NWVAA,NAER+2+NDUST), STAT=RC )
       IF ( RC /= GC_SUCCESS ) THEN
