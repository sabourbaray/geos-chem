...
!ewl#ifdef CLOUDJ
!ewl    USE Cldj_Interface_Mod, ONLY : Init_CloudJ
!ewl#else
...
!
    ...
    !--------------------------------------------------------------------
    ! Initialize Fast-JX / Cloud-J schemes
    !
    ! NOTE: we need to call this for a dry-run so that we can get
    ! a list of all of the lookup tables etc read by Fast-JX/Cloud-J
    !--------------------------------------------------------------------
!ewl#ifdef CLOUDJ
!ewl       IF ( TRIM(Input_Opt%CloudJ_Dir) == MISSING_STR ) THEN
!ewl          ErrMsg = 'Init_Photolysis: Cloud-J directory missing in ' // &
!ewl                   'geoschem_config.yml!'
!ewl          CALL GC_Error( ErrMsg, RC, ThisLoc )
!ewl          RETURN
!ewl       ENDIF
!ewl       CALL Init_CloudJ( Input_Opt, State_Diag, State_Chm, RC )
!ewl       IF ( RC /= GC_SUCCESS ) THEN
!ewl          ErrMsg = 'Error encountered in "Init_CloudJ"!'
!ewl          CALL GC_Error( ErrMsg, RC, ThisLoc )
!ewl          RETURN
!ewl       ENDIF
!ewl#else
...
240             FORMAT( a10, ':', i7, 2x, i7, 2x, f7.4 )
    ...
    IF ( .not. Input_Opt%DryRun ) THEN
       ...
       ! NOTE: MIEDX was mostly not used in Fast-JX. It is used for all
       ! aerosols in Cloud-J (ewl)
...
!ewl#ifdef CLOUDJ
!ewl    USE Cldj_Interface_Mod, ONLY : Run_CloudJ
!ewl#else
...
!ewl#ifdef CLOUDJ
!ewl    CALL Run_CloudJ( Wavelength, Input_Opt, State_Chm, State_Diag, &
!ewl                     State_Grid, State_Met, RC )
!ewl#else
...
!ewl#endif
    ...
    IF ( RC /= GC_SUCCESS ) THEN
!ewl#ifdef CloudJ
!ewl       ErrMsg = 'Error encountered in subroutine Run_CloudJ!'
!ewl#else
       ErrMsg = 'Error encountered in subroutine RunFastJX!'
!ewl#endif
       CALL GC_Error( ErrMsg, RC, ThisLoc )
       RETURN
    ENDIF
...
!
!ewl#ifdef CLOUDJ
!ewl    USE Cldj_Cmn_Mod,   ONLY : AN_, NAA, TITLAA
!ewl#else
