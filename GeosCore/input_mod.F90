...
!cldj    key   = "operations%photolysis%input_directories%cloudj_input_dir"
!cldj    v_str = MISSING_STR
!cldj    CALL QFYAML_Add_Get( Config, TRIM( key ), v_str, "", RC )
!cldj    IF ( RC /= GC_SUCCESS ) THEN
!cldj       errMsg = 'Error parsing ' // TRIM( key ) // '!'
!cldj       CALL GC_Error( errMsg, RC, thisLoc )
!cldj       RETURN
!cldj    ENDIF
!cldj    Input_Opt%CloudJ_DIR = TRIM( v_str )
...
!cldj       WRITE( 6,120 ) 'Cloud-J input directory     : ',                      &
!cldj                       TRIM( Input_Opt%CloudJ_Dir )
